...
//@access: Need Auth (Private)
router.get(
  "/",
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    const errors = {};
    // FIND BASED ON USER ID -> LOOK AT SCHEMA / PROFILE MODEL
...
// (backend API route)
//@desc: Display/get profile by handle
//@access: Public (don't need passport.authenticate)
...
router.get("/handle/:handle", (req, res) => {
  const errors = {};
  //use Profile model, and find one: and grab handle from URL (params)
...
// (backend API route)
//@desc: Display/get profile by user id
//@access: Public (don't need passport.authenticate)
...
router.get("/user/:user_id", (req, res) => {
  ...
  Profile.findOne({ user: req.params.user_id })
    ...
    .then(profile => {
      if (!profile) {
        errors.noprofile = "Can't find a profile for that user";
        res.status(404).json(errors);
      }
      res.json(profile);
    })
...
router.post(
  ...
  (req, res) => {
    ...
    //Skills - Split into array: (comes as CSV)
    if (typeof req.body.skills !== "underfined") {
      profileFields.skills = req.body.skills.split(",");
    }
    //Social (object of fields) Have to set to empty object first.. (if not it wont exist)
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      if (profile) {
        ...
        //Create
        //Check to make sure Handle is unique:
...
//@access: Private (passport.authenticate)
router.post(
  "/experience",
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      const newEdu = {
        ...
        feildofstudy: req.body.location,
        from: req.body.from,
        to: req.body.to,
        current: req.body.current,
        description: req.body.decription
